...
switch lower(x.Q.PulseSequenceType)
    ...
    case 'grase'
        ...
        % and 3d_spiral.
    otherwise
        warning(['Unknown setting x.Q.PulseSequenceType=' xASL_num2str(x.Q.PulseSequenceType)]);
end

%% ----------------------------------------------------------------------------------------
%% 2) Attempt accounting for in-plane interpolation in reconstruction
if ~isempty(regexpi(x.Q.PulseSequenceType, 'spiral', 'once')) && NativeResolution(1)<4
        NativeResolution(1:2) = max(NativeResolution(1:2),[3.75 3.75]);
        % to account for in-plane interpolation
elseif ~isempty(regexpi(x.Q.PulseSequenceType, 'GRASE', 'once')) && NativeResolution(1)<3
