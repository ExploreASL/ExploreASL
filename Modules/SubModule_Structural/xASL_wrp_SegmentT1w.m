...
dirLesionMasks = x.D.FLAIR;
xASL_adm_CreateDir(dirLesionMasks);
for iL=1:length(Lesion_FLAIR_list)
    rLesion_FLAIR_list{iL} = fullfile(dirLesionMasks, ['rLesion_' x.P.FLAIR '_' num2str(iL) '_' x.P.SubjectID '.nii']);
    xASL_spm_deformations(x,Lesion_FLAIR_list{iL}, rLesion_FLAIR_list{iL}, 2);
end
for iL=1:length(Lesion_T1_list)
    rLesion_T1_list{iL} = fullfile(dirLesionMasks, ['rLesion_' x.P.STRUCT '_' num2str(iL) '_' x.P.SubjectID '.nii']);
...
% Combine all lesions
rLesionList = xASL_adm_GetFileList(dirLesionMasks, ['(?i)^rLesion.*' x.P.SubjectID '\.nii'], 'FPList', [0 Inf]);
