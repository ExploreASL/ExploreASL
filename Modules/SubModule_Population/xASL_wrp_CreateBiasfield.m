...
%     Bmask   = (pGMim+pWMim)>0.5;
    ...
    for iSite=1:nSites
        ...
        if xASL_exist(SiteAveragePath,'file')
            ...
            RobustMean(:,:,:,iSite) = xASL_im_Column2IM(xASL_stat_MeanNan(IM(:,NoOutliers), 2),x.utils.WBmask);
        end
    end
...
for iSite=1:nSites
    ...
    for iScan=1:size(IM{iSite},2)
        ...
        xASL_io_SaveNifti(FilePath,FilePath,xASL_im_Column2IM(IM{iSite}(:,iScan),x.utils.WBmask),[],0);
    end
end
