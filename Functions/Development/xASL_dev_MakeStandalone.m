...
mcc('-m', '-v',... % '-R -nodisplay -R -softwareopengl',... % https://nl.mathworks.com/matlabcentral/answers/315477-how-can-i-compile-a-standalone-matlab-application-with-startup-options-e-g-nojvm
	...
	'-a', fullfile(x.opts.MyPath,'Functions','Development'),...
    '-a', fullfile(x.opts.MyPath,'Functions','External'),...
    ...
    '-a', fullfile(x.opts.MyPath,'Maps'),... % Maps
    '-a', fullfile(x.opts.MyPath,'Maps','Templates'),...
    '-a', fullfile(x.opts.MyPath,'Modules'),... % Modules
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Import'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Import', 'DCM2NII'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Import', 'Init'),...
	'-a', fullfile(x.opts.MyPath,'Modules', 'Module_Import', 'NII2BIDS'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Processing'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Processing', 'Module_BIDS2Legacy'),...    
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Processing', 'Module_Structural'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Processing', 'Module_ASL'),...
    '-a', fullfile(x.opts.MyPath,'Modules', 'Module_Processing', 'Module_Population'),...
    '-a', fullfile(x.opts.MyPath,'External'),... % External software
    '-a', fullfile(x.opts.MyPath,'External','Atlases4ROIs'),... % Atlases
    '-a', fullfile(x.opts.MyPath,'External','Atlases4ROIs','LicenseLimited'),...
    '-a', fullfile(x.opts.MyPath,'External','Atlases4ROIs','LicensePermissive'),...
    '-a', fullfile(x.opts.MyPath,'External','Atlases4ROIs','LicensePermissive','SPM_CAT_GPL'),...
    '-a', fullfile(x.opts.MyPath,'External','Atlases4ROIs','LicensePermissive','VascularTerritories'),...
    '-a', fullfile(x.opts.MyPath,'External','bids-matlab'),...
    '-a', fullfile(x.opts.MyPath,'External','DCMTK'),...
    '-a', fullfile(x.opts.MyPath,'External','MRIcron'),... % DICOM import
	'-a', fullfile(x.opts.MyPath,'External','SuperGZip') );
