...
%            - renamed to xASL_bilateralFilter
%            - removed DIP dependency for the erosion
%% variables
...
if nargin<4 || isempty(varargin{2});
  ...
  for ii=1:sz(3)
	mask(:,:,ii)=dip_array(closing(erosion(threshold(xASL_stat_MeanNan(volIM(:,:,ii,:),4)),3),5));
...
for iLabel=1:2
      ...
      % Filter in time
      %MeanV = xASL_im_ndnanfilter(tmp,'gauss',[0 18.84]);
	  MeanV = ndnanfilter(tmp,'GaussWinImage',20,2);
      
      % Add - outlier removal, filtering again
      tmp = tmp - MeanV;
