...
if x.S.InputNativeSpace
	% Native space
	x.S.WBmask = xASL_io_Nifti2Im(fullfile(x.D.ROOT,x.SUBJECTS{1},x.SESSIONS{1},[x.S.InputAtlasNativeName '.nii']));
	x.S.WBmask = sum(x.S.WBmask,4) > 0;
	...
	x.LeftMask = (x.S.WBmask .* (x.LeftMask == 1)) > 0;
else
	% Standard space
	x.LeftMask = x.S.WBmask;
	x.LeftMask(1:60,:,:) = 0;
end
x.LeftMask = xASL_im_IM2Column(x.LeftMask,x.S.WBmask);
...
if x.S.InputNativeSpace
	...
	for kk = 1:atlasN
		x.S.InputMasks(:,kk) = xASL_im_IM2Column(inputAtlasTmp == kk,x.S.WBmask);
	end
...
if x.S.InputNativeSpace
	...
	if isfield(x.S,'MaskSusceptibility') && ~min(x.S.MaskSusceptibility == xASL_im_IM2Column(ones(121,145,121),x.S.WBmask))
	...
	for iSess=1:nSessions
		...
		if x.S.IsASL
			...
			if x.S.InputNativeSpace
				...
				x.S.WBmask = xASL_io_Nifti2Im(fullfile(x.D.ROOT,x.SUBJECTS{iSubject},x.SESSIONS{iSess},[x.S.InputAtlasNativeName '.nii']));
				x.S.WBmask = sum(x.S.WBmask,4) > 0;
				[~,tmpNameLeftRight] = xASL_fileparts(x.P.Path_LeftRightPop);
		...
		if x.S.InputNativeSpace
			if x.S.IsASL
				...
				if xASL_exist(x.P.Path_PVgm,'file')
					pGM = xASL_im_IM2Column(xASL_io_Nifti2Im(x.P.Path_PVgm),x.S.WBmask);
				else
				...
				if xASL_exist(x.P.Path_PVwm,'file')
					pWM = xASL_im_IM2Column(xASL_io_Nifti2Im(x.P.Path_PVgm),x.S.WBmask);
				else
					fprintf('%s\n',[x.P.Path_PVgm ' missing...']);
					continue;
				end
				...
				if xASL_exist(x.P.Path_PVwmh, 'file')
					pWMH = xASL_im_IM2Column(xASL_io_Nifti2Im(x.P.Path_PVwmh), x.S.WBmask);
			...
			if iSess == 1
				if x.S.IsASL
					...
					if xASL_exist(PathGM,'file')
						pGM = xASL_im_IM2Column(xASL_io_Nifti2Im(PathGM),x.S.WBmask);
					else
						fprintf('%s\n',[PathGM ' missing...']);
						continue;
					end
					...
					if xASL_exist(PathWM,'file')
						pWM = xASL_im_IM2Column(xASL_io_Nifti2Im(PathWM),x.S.WBmask);
					else
						fprintf('%s\n',[PathWM ' missing...']);
						continue;
					end
					...
					if xASL_exist(WMHfile,'file')
						...
						pWMH = xASL_im_IM2Column(xASL_io_Nifti2Im(WMHfile), x.S.WBmask);
						% we take sqrt(pWMH) to mimic the effective resolution of ASL (instead of smoothing)
						pWMH(pWMH<0) = 0;
						pGM = max(0, pGM - pWMH);
						pWM = max(0, pWM - pWMH);
		...
		if x.S.InputNativeSpace %% PM: we repeat same code here twice
			...
			if xASL_exist(FilePath,'file')
				...
				DataIm = xASL_im_IM2Column(Data3D,x.S.WBmask);
            end
            ...
            if x.S.bMasking(2)==1
                ...
                if xASL_exist(FilePath,'file')
                    VascularMask = xASL_im_IM2Column(logical(xASL_io_Nifti2Im(FilePath)), x.S.WBmask);
                end
            else
                VascularMask = xASL_im_IM2Column(ones(size(x.S.WBmask)), x.S.WBmask);
            end
		else
		...
		if numel(DataIm)==1
			DataIm = zeros([sum(x.S.WBmask(:)) 1],'single');
		...
		if x.S.SubjectWiseVisualization && ~x.S.InputNativeSpace
			...
			LabelIM = xASL_Convert4D_3D_atlas(xASL_im_Column2IM(SubjectSpecificMasks(:,[1:3:end]), x.S.WBmask));
		...
		SusceptibilityMask = xASL_im_IM2Column(x.S.WBmask, x.S.WBmask); % default = no susceptibility mask
        if x.S.bMasking(1)==1
            if x.S.InputNativeSpace
                if xASL_exist(x.P.Path_MaskSusceptibilityPop)
                    SusceptibilityMask = xASL_im_IM2Column(xASL_io_Nifti2Im(x.P.Path_MaskSusceptibilityPop),x.S.WBmask);
                end
            else
                ...
                elseif strcmpi(x.Sequence,'3D_spiral')
                    ...
                    if xASL_exist(FilePath,'file')
                        SusceptibilityMask = xASL_im_IM2Column(xASL_io_Nifti2Im(FilePath), x.S.WBmask);
                    else
                        if x.S.IsASL
                            fprintf('%s\n',[FilePath ' missing...']);
                        end
...
ROI = xASL_im_Column2IM(ROI, x.S.WBmask); % convert to image, decompress
...
ROI = xASL_im_IM2Column(ROI, x.S.WBmask); % convert back to column, compress
