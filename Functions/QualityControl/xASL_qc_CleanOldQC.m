function [x] = xASL_qc_CleanOldQC(x, ScanType, bRemoveCurrentSession)
%xASL_qc_RemoveOutdatedQC Removes outdated QC parameters, loads QC from JSON and cleans the current session if necessary
%
% FORMAT: [x] = xASL_qc_RemoveOutdatedQC(x, ScanType[, bRemoveCurrentSession)
%
% INPUT:
%   x 	                  - structure containing fields with all information required to run this submodule (REQUIRED)
%   ScanType              - Type of the data - can be 'ASL' or 'func' (REQUIRED)
...
%
% EXAMPLE: x = xASL_qc_RemoveOutdatedQC(x, 'ASL');
...
if xASL_exist(QC_Path, 'file')
	...
	for iScanType = 1:length(ScanTypeList)
		currentScanType = ScanTypeList{iScanType};
		if isfield(oldOutput, currentScanType)
			% Load all subfields for that scantype
			listFields = fieldnames(oldOutput.(currentScanType));
			for iField = 1:length(listFields)
				% Copy only the subfields in ScanType_N
				if ~isempty(regexp(listFields{iField}, [currentScanType '_\d+'], 'once'))
					% But skip the current session unless bCompleteRun is false
					if ~strcmp(listFields{iField}, x.SESSION) || ~bRemoveCurrentSession
						x.Output.(currentScanType).(listFields{iField}) = oldOutput.(currentScanType).(listFields{iField});
					end
				end
			end
		end
