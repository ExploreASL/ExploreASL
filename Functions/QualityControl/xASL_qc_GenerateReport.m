...
% Set defaults
settings = xASL_qc_GenerateReport_sub_defaultSettings();
...
xASL_qc_GenerateReport_sub_parseJson(config, x, [], [], settings);
...
%%================================================================================================================================================
%%================================================================================================================================================
function [settings] = xASL_qc_GenerateReport_sub_parseJson(json, x, figure, line, settings)
        ...
        if strcmp(fields{iField}, 'content') || strcmp(fields{iField}, 'pages')
            for iContent = 1:length(currentField)
                if iscell(currentField(iContent))
                    [settings, line] = xASL_qc_GenerateReport_sub_parseContent(currentField{iContent}, x, figure, line, settings);
                else
                    [settings, line] = xASL_qc_GenerateReport_sub_parseContent(currentField(iContent), x, figure, line, settings);
                end
            end
        else
            [settings, line] = xASL_qc_GenerateReport_sub_parseContent(currentField, x, figure, line, settings);
        end 
    end
end


function [settings, line] = xASL_qc_GenerateReport_sub_parseContent(currentField, x, figure, line, settings)

    if ~isstruct(currentField)
%        fprintf('xASL_qc_GenerateReport_sub_parseContent couldnt find struct of printable content \n');
        return;
    end
    ...
    switch currentField.type
        case 'text' 
            line = xASL_qc_GenerateReport_sub_PrintText(currentField, figure, line, settings);
        case 'qc'
            line = xASL_qc_GenerateReport_sub_PrintQC(currentField, x, figure, line, settings);
        case 'image'
            settings = xASL_qc_GenerateReport_sub_PrintImage(currentField, x, figure, settings);  
        case 'scan'
            settings = xASL_qc_GenerateReport_sub_PrintScan(currentField, x, figure, settings);
        case 'page'
            xASL_qc_GenerateReport_sub_printPage(currentField, x, settings);  
        case 'block'
            xASL_qc_GenerateReport_sub_printBlock(currentField, x, figure, settings);
        case 'settings'
            settings = xASL_qc_GenerateReport_sub_loadSettings(currentField, settings);  
        case 'patients' 
            line = xASL_qc_GenerateReport_sub_PrintPatient(x, figure, line, settings);
    end  
end

function  xASL_qc_GenerateReport_sub_printPage(json, x, settings)
    ...
    xASL_qc_GenerateReport_sub_PrintImage(logoPath, [], primaryFig, settings, [0 0.96 1 0.04]);

    %% Print the Footer
    xASL_qc_GenerateReport_sub_PrintText('This report was automatically generated by ExploreASL', primaryFig, [0 0.02 1 0], settings);

    %% Parse JSON and create the file
    xASL_qc_GenerateReport_sub_parseJson(json, x, primaryFig, [0 0.93 1 0], settings);
...
function xASL_qc_GenerateReport_sub_printBlock(input, x, pageFig, settings)
    position = xASL_str2num(input.position);
    size = xASL_str2num(input.size);
    [settings.canvas, line] = xASL_qc_GenerateReport_sub_createNewCanvas(position, size, settings.canvas);
    xASL_qc_GenerateReport_sub_parseJson(input, x, pageFig, line, settings);
end

function line = xASL_qc_GenerateReport_sub_PrintPatient(x, figure, line, settings)
    ...
    line = xASL_qc_GenerateReport_sub_PrintText('Participant Information', figure, line, settings );
    for iEntry = 1:size(PatientInfo, 2)
        if strcmp(PatientInfo{1, iEntry}, 'participant_id')
            line = xASL_qc_GenerateReport_sub_PrintText(['Participant: ', PatientInfo{2, iEntry}], figure, line, settings );
        elseif strcmp(PatientInfo{1, iEntry}, 'age')
            line = xASL_qc_GenerateReport_sub_PrintText(['Participant age: ', num2str(PatientInfo{2, iEntry})], figure, line, settings );
        elseif strcmp(PatientInfo{1, iEntry}, 'sex')
            line = xASL_qc_GenerateReport_sub_PrintText(['Participant sex: ', PatientInfo{2, iEntry}], figure, line, settings );
        end
    end

end

function line = xASL_qc_GenerateReport_sub_PrintQC(json, x, figure, line, settings)
    ...
    if isfield(json, 'settings')
        settings = xASL_qc_GenerateReport_sub_loadSettings(json.settings, settings);
    end
    ...
    line = xASL_qc_GenerateReport_sub_PrintText(TextString, figure, line, settings);

end

function [settings] = xASL_qc_GenerateReport_sub_PrintImage(input, x, figure, settings, position)
    switch nargin
    case 4
        ...
        elseif isfield(input, 'absolutePath')
            ...
            ImagePath = xASL_qc_GenerateReport_sub_WildcardReplace(ImagePath, x);
        ...
        elseif isfield(input, 'subjPath')
            ...
            warning('xASL_qc_GenerateReport_sub_PrintImage didnt have a defined path') ;
            ImagePath = xASL_qc_GenerateReport_sub_WildcardReplace(ImagePath, x);
            return
        end
    ...
    [canvas] = xASL_qc_GenerateReport_sub_createNewCanvas(position(1:2), position(3:4), settings.canvas);
    ...
    clear fg
    settings.figureCount = xASL_qc_GenerateReport_sub_PrintHeader(header, figure, settings, canvas);
end

function [strout] = xASL_qc_GenerateReport_sub_WildcardReplace(strin, x)
    strout = strin;
...
function [settings] = xASL_qc_GenerateReport_sub_PrintScan(input, x, figure, settings)
    position = [xASL_str2num(input.position) xASL_str2num(input.size)];
    [canvas] = xASL_qc_GenerateReport_sub_createNewCanvas(position(1:2), position(3:4), settings.canvas);
    ...
    if isfield(input, 'slice')
        [x.S.TraSlices] = xASL_qc_GenerateReport_sub_getSliceFromStruct(input.slice, 'TraSlice');
        [x.S.CorSlices] = xASL_qc_GenerateReport_sub_getSliceFromStruct(input.slice, 'CorSlice');
        [x.S.SagSlices] = xASL_qc_GenerateReport_sub_getSliceFromStruct(input.slice, 'SagSlice');       
    else
...
function [slice] = xASL_qc_GenerateReport_sub_getSliceFromStruct(struct, name)
...
function [newCanvas, line] = xASL_qc_GenerateReport_sub_createNewCanvas(position, size, oldCanvas)
...
function line = xASL_qc_GenerateReport_sub_PrintText(input, figure, line, settings)
    ...
    switch class(input)
        ...
        case 'struct'
            ...
            if isfield(input, 'settings')
                settings = xASL_qc_GenerateReport_sub_loadSettings(input.settings, settings);
            end
        otherwise
            class(input)
            error('xASL_qc_GenerateReport_sub_PrintText couldnt find string of printable text')
    end
    ...
    line = xASL_qc_GenerateReport_sub_NewLine(line, settings);
end

function line = xASL_qc_GenerateReport_sub_NewLine(line, settings)
...
function [figureCount] = xASL_qc_GenerateReport_sub_PrintHeader(header, figure, settings, position)
    ...
    xASL_qc_GenerateReport_sub_PrintText(text, figure, position, settings);
end

function [settings] = xASL_qc_GenerateReport_sub_loadSettings(json, settings)
...
function [settings] = xASL_qc_GenerateReport_sub_defaultSettings()
    settings.color = 'k';
