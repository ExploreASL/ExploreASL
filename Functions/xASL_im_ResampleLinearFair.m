...
elseif newsize(1)==size(im_input,1) && newsize(2)==size(im_input,2) && newsize(3)~=size(im_input,3)
    ...
    output_res = xASL_im_ResampleLinearFair_1D_3(old_res,new_res);
elseif newsize(1)==size(im_input,1) && newsize(2)~=size(im_input,2) && newsize(3)==size(im_input,3)
    % 1-dimensional change second dimension
    output_res = xASL_im_ResampleLinearFair_1D_2(old_res,new_res);
elseif newsize(1)~=size(im_input,1) && newsize(2)==size(im_input,2) && newsize(3)==size(im_input,3)
    % 1-dimensional change first dimension
    output_res = xASL_im_ResampleLinearFair_1D_1(old_res,new_res);
else
...
for k=1:size(new_res,3)
    for j=1:size(new_res,2) % prepare new_res blocks
        for i=1:size(new_res,1) % prepare new_res blocks
            ...
            % Display progress
            percentage      =((k-1)*size(new_res,2)*size(new_res,1)+(j-1)*size(new_res,1)+i ) / ...
...
%% 1-dimensional change third dimension
function output_res = xASL_im_ResampleLinearFair_1D_3(old_res,new_res)
...
for k=1:size(new_res,3)
    ...
    if  floor(start3)<ceil(start3)                              % if there is a head3 (heading partial voxel)
        ...
        weight          =weight      + h_weight3;               % add weight to totalweight
    end
...
%% 1-dimensional change second dimension
function output_res = xASL_im_ResampleLinearFair_1D_2(old_res,new_res)
...
for k=1:size(new_res,2)
    ...
    if  floor(start2)<ceil(start2)                              % if there is a head2 (heading partial voxel)
        ...
        weight          =weight      + h_weight2;               % add weight to totalweight
    end
...
%% 1-dimensional change first dimension
function output_res = xASL_im_ResampleLinearFair_1D_1(old_res,new_res)
...
for k=1:size(new_res,1)
    ...
    if  floor(start1)<ceil(start1)                              % if there is a head1 (heading partial voxel)
        ...
        weight          =weight      + h_weight1;               % add weight to totalweight
    end
