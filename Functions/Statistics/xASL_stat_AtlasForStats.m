...
if ~(size(x.S.InputMasks,4)==1 && maxAtlas>1)
	...
	x.S.InputMasks = false([size(AtlasIn,1),size(AtlasIn,2),size(AtlasIn,3),maxAtlas]);
	for iL=1:maxAtlas
...
if x.S.SubjectWiseVisualization
    ...
    for iSub=1:size(x.S.InputMasks,5)
        xASL_TrackProgress(iSub,size(x.S.InputMasks, 5));
