...
try
    ...
    NativeSpaceFiles{2} = {'.*\.topup.*log' '.*FSL.*' 'WADQC.*\.json' '.*(rep_|BeforeSpikeExclusion|despiked).*' '(VascularArtifact|xASL_qc|qcdc).*' 'rp_ASL4D\.txt' '(ASL4D|M0|.*_sn)\.mat'...
        '(MaskASL|Mean_CBF_Template|mean_PWI_Clipped|PVgm|PVwm|PVt1|PVLesion|PVROI|CBFgm|CBFwm|PVcsf|PVwmh|FoV|M0_biasfield|(m|)mean_control|(mean|)PWI|SD|SNR|TopUp.*|(Mask_|Raw)Template|ATT_bias|B0|(Mean_|)(q|)CBF)(\.json|\.mat|\.nii|\.nii\.gz)$'...
        '(SliceGradient(_extrapolated|)|BrainMaskProcessing|ABV|ATT|Tex|ITT|slice_gradient|Lesion|ROI|rgrey|wASL4D|PWI|PWI3D|PWI4D|y_ASL|Pseudo(CBF|Tissue)|rM0|Field|Unwarped|rASL4D)(\.json|\.mat|\.nii|\.nii\.gz)$'...
