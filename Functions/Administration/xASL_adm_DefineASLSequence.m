...
%   xQ.readoutDim   - dimensionality of readout (2D or 3D) (OPTIONAL)
...
%   xQ.Sequence    - sequence type (readout)
% -----------------------------------------------------------------------------------------------------------------------------------------------------
% DESCRIPTION: This ExploreASL function tries to check what ASL sequence is
% being processed, if this was not already defined in xQ.Sequence.
% It does so by checking known combinations of readout dimensionality
% (xQ.readoutDim) and Vendor, knowing the product sequences of the Vendors.
%
...
%% Check quantification fields readoutDim & xQ.Sequence
if nargin<2 || isempty(bVerbose)
    bVerbose = true;
end

if ~isfield(xQ, 'readoutDim') || isempty(xQ.readoutDim)
    warning('xQ.readoutDim parameter missing');
end

% Check for illegal sequence definitions
if isfield(xQ,'Sequence')
    if isempty(regexpi(xQ.Sequence, '(2d_epi|3d_grase|3d_spiral)'))
        warning(['Unknown ASL sequence: ' xASL_num2str(xQ.Sequence)]);
        fprintf('%s\n', 'Trying to fix this');
        xQ = rmfield(xQ, 'Sequence');
...
if isfield(xQ,'Sequence') && (~isfield(xQ, 'Vendor') || isempty(xQ.Vendor))
    if strcmpi(xQ.Sequence,'3d_spiral')
...
elseif  isempty(regexpi(xQ.Vendor, 'Gold Standard Phantoms|GE|Philips|Siemens'))
    warning('Unknown Vendor specified in xQ.Vendor');
elseif ~isempty(regexpi(xQ.Vendor, 'Gold Standard Phantoms'))
...
if ~isfield(xQ, 'Sequence') && isfield(xQ, 'readoutDim') 
	if strcmpi(xQ.readoutDim,'2D')
		xQ.Sequence = '2D_EPI';
        if bVerbose
            if ~isempty(regexpi(xQ.Vendor,'Gold Standard Phantoms'))
	...
	elseif isfield(xQ, 'Vendor') && strcmpi(xQ.readoutDim,'3D')
		if  ~isempty(regexpi(xQ.Vendor, 'Philips')) || ~isempty(regexpi(xQ.Vendor, 'Siemens'))
			xQ.Sequence = '3D_GRASE'; % assume that 3D Philips or Siemens is 3D GRASE
			if bVerbose; fprintf('%s\n', '3D readout detected with vendor Philips or Siemens, assuming 3D GRASE'); end
		elseif ~isempty(regexpi(xQ.Vendor, 'GE'))
			xQ.Sequence = '3D_spiral'; % assume that 3D GE is 3D spiral
			if bVerbose; fprintf('%s\n', '3D readout detected with vendor GE, assuming 3D spiral'); end
		elseif ~isempty(regexpi(xQ.Vendor, 'Gold Standard Phantoms'))
			xQ.Sequence = '3D_GRASE'; % assume that this is simulated 3D GRASE by the DRO
            if bVerbose
...
if ~isfield(xQ,'Sequence') || isempty(xQ.Sequence)
    warning('No xQ.Sequence defined');
    fprintf('If there are multiple sequence types, this needs to be implemented yet here\n');
    fprintf('Otherwise, please define xQ.Sequence\n');
    fprintf('Setting xQ.Sequence=3D spiral to fool the pipeline here\n');
    ...
    xQ.Sequence = '3D_spiral';
    xQ.readoutDim = '3D';
else
    if bVerbose; fprintf('%s\n', [xQ.Sequence ' sequence detected']); end
end
