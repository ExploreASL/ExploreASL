...
%
% FORMAT:       xASL_adm_GetListCompletedModules(x)
% 
...
%
% -----------------------------------------------------------------------------------------------------------------------------------------------------
% EXAMPLE:      xASL_adm_GetListCompletedModules(x);
...
for iList = 1:length(listStructSubjects)
    currentDir = fullfile(dirLockStruct, listStructSubjects{iList});
    lockFile999 = fullfile(currentDir, 'xASL_module_Structural', '999_ready.status');
...
for iList = 1:length(listASLSubjects)
    currentDir = fullfile(dirLockASL, listASLSubjects{iList});
    sessions = xASL_adm_GetFileList(currentDir, '^xASL_module_ASL_.*', 'List', [], 1);
    for iSession=1:length(sessions)
        currentDir = fullfile(dirLockASL, listASLSubjects{iList}, sessions{iSession});
        lockFile999 = fullfile(currentDir, '999_ready.status');
        if exist(lockFile999, 'file')
