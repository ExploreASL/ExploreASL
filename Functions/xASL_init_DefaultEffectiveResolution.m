...
%   x.Sequence  - type of ASL readout used (2D_EPI, 3D GRASE or 3D spiral)
%
% OUTPUT:
...
if ~isfield(x,'Sequence')
    warning(['Setting x.Sequence missing, skipping']);
    return;
end
...
switch lower(x.Sequence)
    ...
    case '3d_grase'
        ...
        % and 3d_spiral.
    otherwise
        warning(['Unknown setting x.Sequence=' xASL_num2str(x.Sequence)]);
end
...
fprintf('%s\n',[x.Sequence ' NIfTI has native resolution ' num2str(NativeResolution(1)) ' ' num2str(NativeResolution(2)) ' ' num2str(NativeResolution(3)) ', assuming PSF ' num2str(Estimated_FWHM(1)) ' ' num2str(Estimated_FWHM(2)) ' ' num2str(Estimated_FWHM(3)) ' this gives estimated effective resolution ' num2str(EffectiveResolution(1)) ' ' num2str(EffectiveResolution(1)) ' ' num2str(EffectiveResolution(3))])
