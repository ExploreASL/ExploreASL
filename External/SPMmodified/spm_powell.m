...
while t(2).f > t(3).f
    ...
    tmp = cat(1,t.p)-t(2).p;
	% ExploreASL fix: 2018-08-16 Guillaume Flandin to solve issues with inter-version repeatability
	%pol = pinv([ones(3,1) tmp tmp.^2])*cat(1,t.f);
    pol = [ones(3,1) tmp tmp.^2] \ cat(1,t.f);
    % ExploreASL fix: 2019-05-25 Jan Petr - round to solve the issue with
    % Win-Linux repeatability
    %pol = xASL_round([ones(3,1) tmp tmp.^2] \ cat(1,t.f),18);
