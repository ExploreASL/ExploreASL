...
% ______________________________________________________________________
% Christian Gaser
% $Id: cat_run_job.m 1343 2018-08-02 08:41:03Z dahnke $
...
%#ok<*WNOFF,*WNON>
	...
	% deprecated, now using job.extopts.xasl_quality parameter
    % get job defaults, ExploreASL hack to speed up for reproducibility tests
    %if  exist(fullfile(pth,'LowQ.status'),'file')
    %    LowQ    = true;
    %else
    %    LowQ    = false;
    %end    
        ...
        for n=1:numel(job.channel) 
            ...
            if (job.extopts.APP==1 || job.extopts.APP==2) && ~ppe.affreg.skullstripped
              ...
              for ix=1:numel(sampx)
                ...
                stime2 = cat_io_cmd(sprintf('  SPM bias correction (samp: %0.2f mm, fwhm: %3.0f mm)\n',preproc.warp.samp,...
