function [Ym,Ybg,WMth,bias] = cat_run_job_APP_SPMinit1585(job,tpm,ppe,n,ofname,nfname,mrifolder,skullstripped)
...
%%
      ...
      if ix==numel(sampx) %|| (bias(1) > preproc.channel.biasfwhm && ix>1)
        ...
        Yw  = cat_vol_approx1585(max(0,min(2,Yw)),'',vx_vol,2,struct('lfO',2));
...
Yoc(isnan(Yo)) = nan; % not sure if this is good
  ...
  if exist('vout','var') && job.extopts.APP==2 && isfield(vout,'Ycls')
    ...
    try
      ...
      [Yo,Ym] = cat_run_job_APP_SPMfinal1585(onfname,vout,vx_vol * (2 - strcmp(job.extopts.species,'human')),...
