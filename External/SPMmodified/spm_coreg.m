...
if ~isfield(VG, 'uint8')
	...
	%%% ExploreASL fix
	ResStr1  = ['[' num2str(vxg(1),3) ' ' num2str(vxg(2),3) ' ' num2str(vxg(3),3) ']'];
    ResStr2  = ['[' num2str(fwhmg(1),3) ' ' num2str(fwhmg(2),3) ' ' num2str(fwhmg(3),3) ']'];
    disp(['Smoothing ' ResStr1 ' into ' ResStr2 ' FWHM resolution']);
...
for k=1:numel(VF)
    ...
    if ~isfield(VFk, 'uint8')
		...
		ResStr1  = ['[' num2str(vxg(1),3) ' ' num2str(vxg(2),3) ' ' num2str(vxg(3),3) ']'];
        ResStr2  = ['[' num2str(fwhmg(1),3) ' ' num2str(fwhmg(2),3) ' ' num2str(fwhmg(3),3) ']'];
        disp(['Smoothing ' ResStr1 ' into ' ResStr2 ' FWHM resolution for scan ' num2str(k)]);
	...
	fprintf(', ');%%% ExploreASL fix
%     if flags.graphics %%% ExploreASL fix
%         display_results(VG(1),VFk(1),xk(:)',flags); %%% ExploreASL fix
%     end %%% ExploreASL fix
end
fprintf('\n');%%% ExploreASL fix
fprintf('%-40s: %30s\n','Completed',spm('time'))                        %-#
...
lim  = ceil(2*fwhm);
krn1 = spm_smoothkern(fwhm(1),-lim(1):lim(1)) ; krn1 = krn1/sum(krn1); 
krn2 = spm_smoothkern(fwhm(2),-lim(2):lim(2))'; krn2 = krn2/sum(krn2);
H2 = zeros(size(H)+28);
H2(14+(1:size(H,1)),14+(1:size(H,2))) = H;
H = H2;
H = xASL_im_conv3Dsep(double(H),krn2,krn1,0);
