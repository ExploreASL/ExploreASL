...
if  isfield(x,'ForceInclusionList')
    ...
    x.TotalSubjects    = sort(xASL_adm_GetFsList(x.D.ROOT, x.subject_regexp ,true));
end
...
if ~isfield(x,'SESSIONS') % default session
    x.SESSIONS{1}     = 'ses-ASL01';
end
...
%         x.SESSIONS          = '';
%         rootpath = fullfile(x.D.ROOT,x.TotalSubjects{iSubj});
%         SessionPathList{iSubj}     = GetFileList(rootpath, '^ses-ASL\d*$', 'FPListRec', [0 Inf],1);
%         for iSess=1:length(SessionPathList{iSubj})
%             [~, x.SESSIONS{end+1}]  = fileparts(SessionPathList{iSubj}{iSess});
%         end
%         x.SESSIONS          = unique(x.SESSIONS);
%         if  isempty(x.SESSIONS)
%             x.SESSIONS{1}       = 'ses-ASL01'; % default session
%         end
%     end
% end
...
if  sum(ListNoPipelineDir)>0
    ...
    for iS=1:length(TotalSubjectsTemp)
        if  ListNoPipelineDir(iS)
            x.TotalSubjects{nNext}      = TotalSubjectsTemp{iS};
            nNext                       = nNext+1;
        end
    end
end
