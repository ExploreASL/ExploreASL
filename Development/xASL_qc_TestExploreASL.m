...
for iList=1:length(Dlist) % iterate over example datasets
    ...
    if ~isempty(PathCBF)
        ResultsTable{1+iList,5+length(ResultFile)} = xASL_qc_TanimotoCoeff(PathCBF{1}, PathTemplateASL, x.utils.WBmask, 3, 0.975, [4 0]); % Tanimoto Coefficient, Similarity index
    end
    if ~isempty(PathM0)
        ResultsTable{1+iList,6+length(ResultFile)} = xASL_qc_TanimotoCoeff(PathM0{1}, PathTemplateM0, x.utils.WBmask, 3, 0.975, [4 0]); % Tanimoto Coefficient, Similarity index
    end
end
fprintf('\n');
